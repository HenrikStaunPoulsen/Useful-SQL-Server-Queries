...
(
	SELECT TOP 100
		CAST(SUM(s.total_elapsed_time) / 1000000.0 AS DECIMAL(18, 2)) AS [Total Elapsed Time in S],
		SUM(s.execution_count) AS [Total Execution Count],
		CAST(SUM(s.total_worker_time) / 1000000.0 AS DECIMAL(18, 2)) AS [Total CPU Time in S],
		CAST(SUM(s.total_worker_time) / SUM(s.execution_count) / 1000.0 AS DECIMAL(18, 2)) AS [Avg CPU Time in MS],
		SUM(s.total_logical_reads) AS [Total Logical Reads],
		CAST(CAST(SUM(s.total_logical_reads) AS FLOAT) / CAST(SUM(s.execution_count) AS FLOAT) AS DECIMAL(18, 2)) AS [Avg Logical Reads],
		SUM(s.total_logical_writes) AS [Total Logical Writes],
		CAST(CAST(SUM(s.total_logical_writes) AS FLOAT) / CAST(SUM(s.execution_count) AS FLOAT) AS DECIMAL(18, 2)) AS [Avg Logical Writes],
		SUM(s.total_clr_time) AS [Total CLR Time],
		CAST(SUM(s.total_clr_time) / SUM(s.execution_count) / 1000.0 AS DECIMAL(18, 2)) AS [Avg CLR Time in MS],
		CAST(SUM(s.min_worker_time) / 1000.0 AS DECIMAL(18, 2)) AS [Min CPU Time in MS],
		CAST(SUM(s.max_worker_time) / 1000.0 AS DECIMAL(18, 2)) AS [Max CPU Time in MS],
		...
		CAST(SUM(s.min_clr_time) / 1000.0 AS DECIMAL(18, 2)) AS [Min CLR Time in MS],
		CAST(SUM(s.max_clr_time) / 1000.0 AS DECIMAL(18, 2)) AS [Max CLR Time in MS],
